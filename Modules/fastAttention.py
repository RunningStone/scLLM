...
from scLLM.Modules.utils import default, exists, empty, cast_tuple, null_context
...
class SelfAttention_lora(nn.Module):
    def __init__(
        self,
        dim,
        causal = False,
        ...
        # replace nn.Linear with Linear for lora training
        self.to_q = LoRALinear(dim, inner_dim, bias = qkv_bias)
        self.to_k = LoRALinear(dim, inner_dim, bias = qkv_bias)
        self.to_v = LoRALinear(dim, inner_dim, bias = qkv_bias)
        self.to_out = LoRALinear(inner_dim, dim)
        self.dropout = nn.Dropout(dropout)
